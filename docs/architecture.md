...
## Run artifact output format (`.helioframe/runs/`)

Each execution writes an immutable run directory under:

`.helioframe/runs/<run-id>/`

Key files/directories:

- `manifest.json`: high-level run metadata, selected backend/preset, stage timings, and computed windows.
- `artifacts/intermediate/probe.json`: machine-readable probe output from the `probe` stage.
- `artifacts/intermediate/shots.json`: machine-readable scene-boundary detection output from the `shots` stage.
- `artifacts/intermediate/windows.json`: machine-readable temporal window plan from the `window` stage.
- `artifacts/intermediate/anchors.json`: machine-readable anchor-frame sets generated by the `anchors` stage.
- `artifacts/intermediate/window_batches.json`: machine-readable window batch scheduling details.

The planning-stage artifacts are persisted on every run so debugging can inspect stage inputs/outputs directly without rerunning the full stack.
